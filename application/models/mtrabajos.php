...
public  function tblexcel($param){
	...
	public  function tbltitle($param){
		...
		$this->db->select('pr.idproceso as PROCESO,pd.factura AS FACTURA,p.nomprod AS PRODUCTO,pd.descripcion AS DESCRIPCION,pr.cantidad AS CANTIDAD,pr.precio1 AS PRECIO,(sum(bp.cantidad)*pr.cantidad) as NUMBORDADOS,prebordado AS PREBORDADO,pr.fecha AS FECHA,pe.nombres AS OPERARIO');
		$this->db->from('periodo x,proceso pr');
		$this->db->join('producto p','p.id_prod=pr.id_prod');
		$this->db->join ('bordadosproductos bp','p.id_prod = bp.id_prod');
		$this->db->join(' bordados b ', 'b.idbordados = bp.idbordados');
