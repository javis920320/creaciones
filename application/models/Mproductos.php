...
public  function getproductos($param){
		...
		$query=$this->db->query("select p.id_prod,tp.nomtipoprod,p.nomprod,pr.valor,pr.subvalor,precionocturno,pr.valorsatelite,sum(bp.cantidad)'nbordados',sum(b.precio*bp.cantidad)'vbordado'
 from producto p
...
public  function valorprenda($arreglo){
		...
		$datos = array(
			...
			'precionocturno'=>$arreglo['precionocturno'],
...
public function asignarprecio($param){
 ...
 $datos = array('idprecio' =>null,'estado' =>1,'fecha'=>$param['fecha'],'valor'=>$param['valor'],'subvalor'=>$param['subvalor'],'valorsatelite'=>$param['valorsatelite'],'precionocturno'=>$param['precionoc'],'id_prod'=>$param['id_prod']);
