...
$query=$this->db->query("select pr.idproceso, pe.factura,tp.nomtipoprod,pro. nomprod,pe. talla,pe. facultad, pr.cantidad,pr.cantidad as procesado,per.nombres as cliente,pe.fecha_ingreso,pe.fentrega,pr.fecha,(select p2.nombres from persona p2 inner join trabajador t2 on t2.idpersona=p2. idpersona where t2.idtrabajador=pr.idtrabajador )as trabajador,pe.descripcion
from proceso pr
