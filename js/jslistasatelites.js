        ...
        'columns': [
            ...
            { data: 'prebordado' },
            {
                ...
                render: function(data, type, row) {
                    ...
                    return '<span class="pull-right"> $ ' + row.precio + '</span>';
                ...
                render: function(data, type, row) {
                    ...
                    return '<div><button style="float:left"  class="btn btn-success" data-toggle="modal" data-target="#editarSatelite"  onClick="editarSatelite(\'' + row.nombres + '\',\'' + row.idtrabajador + '\',\'' + row.idproceso + '\');"> <span class="glyphicon glyphicon-transfer"> </span><span class="glyphicon glyphicon-user"></span></button></div>';
    ...
    'columns': [
        ...
        { data: 'fecha' },
        {
            ...
            render: function(data, type, row) {
                ...
                return '<button class=" report btn btn-success"> <i class="glyphicon glyphicon-download-alt"></i></button> <button class="btn btn-danger" onclick="generarPDF(' + row.idtrabajador + ',\'' + row.fecha + '\')"> <i class="glyphicon glyphicon-download-alt"></i>PDF</button>';
            }
        }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(obj, function(i, items) {
                ...
                html += '<div id="mycontenedor" onmouseover="verdetalles();"><span><strong>' + items.nombres + '</strong></span><h4><code>' + items.fecha + '</code></h4><h5>Productos Asignados(' + items.cantidad + ')</h5></div>';
            });
...
editarSatelite = function(nombres, idtrabajador, idproceso) {


    $('#trabajador1').val(idtrabajador);
    $('#idsateliteE').val(idproceso);
...
function cambioAsg() {
    ...
    var idtrabajador = $('#trabajador1').val();
    var idproceso = $('#idsateliteE').val();


    $.ajax({
        url: baseurl + 'Ctaller/cambioAsignacionProceso',
        data: { idtrabajador: idtrabajador, idproceso: idproceso },
        type: 'POST',
        success: function(data) {
            if (data === 0) {

            } else {
                alert('Se realizo el cambio de Asignacion');
                $('#tblsatelite1').DataTable().ajax.reload();
            }
...
function generarPDF(idtrabajador, fecha) {

    window.open(baseurl + 'Cpdfreport/reporte/' + idtrabajador + '/' + fecha);
